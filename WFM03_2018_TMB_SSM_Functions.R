...
# DATA SECTION
############################################################################
setdata <- function(retroyear=2017,ESS_W_T=1,ESS_W_G=1,agecompfit=1)
{
  fyear <- 1986
  lyear <- 2017
  lryear <- retroyear
  ...
  # Aval = 0.05;  #essentially the sd of largest values
  # Bval = 7.95;  #essentially A+B is the sd of the smallest values
  # Cval = 0.0004;
...
############################################################################
setsimdata <- function(retroyear=2017,ESS_W_T=1,ESS_W_G=1,agecompfit=1)
{
  fyear <- 1986
  lyear <- 2017
  lryear <- retroyear
  ...
  in_NtildeT <- round(abs(rnorm(length(years),484.9375,265.0662653)))
  in_N_SampT <- round(in_NtildeT*ESS_W_T)
  
  in_obs_PAT <- matrix(0,nrow=length(years),ncol=length(ages))
  # for(i in 1:length(in_NtildeT)){in_obs_PAT[i,] <- t(rmultinom(1,size=(in_N_SampT[i]+1),prob=c(0.05,0.12,0.2,0.15,0.12,0.1,0.08,0.08,0.04,0.03,0.02,0.01))/(in_N_SampT[i]+1))}
  for(i in 1:length(in_NtildeT)){in_obs_PAT[i,] <- t(rmultinom(1,size=(in_N_SampT[i]+1),prob=c(0.10957363,0.179285259,0.191316359,0.181900851,0.145166935,0.093661329,0.048852514,0.025727803,0.01202485,0.004149948,0.003728078,0.004612442))/(in_N_SampT[i]+1))}
  ...
  in_harv_wgtT <- c(118065,174693,201989,189454,213174,206636,274034,294428,
                    ...
                    307347,271060,228608,233297,235096,188957,227311,427777,
                    241548,287277,229570,158778,124146,69404,71789,49150)
  # for(i in 1:length(in_harv_wgtT)){in_harv_wgtT[i]<-rlnorm(1,log(in_harv_wgtT[i]),sdlog=0.0667612)}
  ...
  in_NtildeG <- round(abs(rnorm(length(years),297.78125,193.458962)))
  in_N_SampG <- round(in_NtildeG*ESS_W_G)
  
  in_obs_PAG <- matrix(0,nrow=length(years),ncol=length(ages))
  # for(i in 1:length(in_NtildeG)){in_obs_PAG[i,] <- t(rmultinom(1,size=(in_N_SampG[i]+1),prob=c(0.05,0.12,0.2,0.15,0.12,0.1,0.08,0.08,0.04,0.03,0.02,0.01))/(in_N_SampG[i]+1))}
  for(i in 1:length(in_NtildeG)){in_obs_PAG[i,] <- t(rmultinom(1,size=(in_N_SampG[i]+1),prob=c(0.038746806,0.144640891,0.182788662,0.16398313,0.144156123,0.13492822,0.089051863,0.05362918,0.026752263,0.011852575,0.006295057,0.00317523))/(in_N_SampG[i]+1))}
  ...
  in_harv_wgtG <- c(132548,212341,284612,289297,218073,318960,
                    ...
                    132603,131388,207987,101251,128992,
                    42136,70174,48190)
  # for(i in 1:length(in_harv_wgtG)){in_harv_wgtG[i]<-rlnorm(1,log(in_harv_wgtG[i]),sdlog=0.0667612)}
...
#############################################################################
setparam <- function(data,agecompfit,log_sig = -2.70663320551,log_sdSR = -1.279150914,
                     log_sd_logeffortT = -0.99251027,log_sd_logeffortG = -1.128061467,
                     logit_rhoalphaT = 0,logit_rhoalphaG = 0,
                     logit_rhoalphacatchT = 0,logit_rhoalphacatchG = 0)
{
  ...
  log_sig <- log_sig #Equivalent of variance = 0.06 (-2.70663320551) for obs error
  log_sig <- -2.120264
  log_sdSR <-   log_sdSR
  ...
  # log_sd_logcatchT <- sqrt((exp(log_sig))^2*length(data$asymptoteages))
  # log_sd_logcatchG <- sqrt((exp(log_sig))^2*length(data$ages))
  # log_sd_logcatchT <- -1.320339 #4 times SD for non-age specific years
  # log_sd_logcatchG <- -1.320339
  # log_sd_logcatchT <- -0.9148737 #6 times SD for non-age specific years
  # log_sd_logcatchG <- -0.9148737
  
  # log_sd_logcatchT <- 1.386294
  # log_sd_logcatchG <- 1.386294
  log_sd_logcatchT <- log(c(1.4065612,1.1544384,0.8182905,0.9669394,1.1520390,1.1270277,1.0214694,0.9388989,0.8748424,0.7256364,0.7570139,0.6138202))
  log_sd_logcatchG <- log(c(1.2020518,1.6034922,1.5177070,1.2049372,0.9671414,0.9288219,0.9682891,1.0033195,1.0965579,1.0338046,0.8762943,0.8809800))
  ...
  if(agecompfit==2){
    return(parameters <- list(log_sig=log_sig,
                              log_sdSR=log_sdSR,
                              log_sd_logeffortT=log_sd_logeffortT,
                              log_sd_logeffortG=log_sd_logeffortG,
                              logit_rhoalphaT=logit_rhoalphaT,
  ...
  if(agecompfit==3){
    return(parameters <- list(log_sig=log_sig,log_sdSR=log_sdSR,
                              ...
                              log_qG=log_qG,
                              log_rec=log_rec,
                              log_thetaT=log_thetaT,
                              log_thetaG=log_thetaG))
  }
...
setbounds <- function(agecompfit)
{
  ...
  log_sd_logcatchT_upper <- rep(5,12)
  log_sd_logcatchG_upper <- rep(5,12)
  ...
  if(agecompfit==2)
  {
    upper_bounds <- c(log_sig_upper=log_sig_upper,
                      ...
                      lnM_upper=lnM_upper,
                      Aval_upper=Aval_upper,
                      Bval_upper=Bval_upper,
                      Cval_upper=Cval_upper)
  }
  if(agecompfit==3)
  {
    upper_bounds <- c(#log_sig_upper=log_sig_upper,
                      ...
                      logit_rhoalphaG_upper=logit_rhoalphaG_upper,
                      lnM_upper=lnM_upper,
                      log_thetaT_upper=log_thetaT_upper,
                      log_thetaG_upper=log_thetaG_upper)
  }
  ...
  log_sd_logcatchT_lower <- rep(-20,12)
  log_sd_logcatchG_lower <- rep(-20,12)
  ...
  if(agecompfit==2)
  {
    lower_bounds <- c(log_sig_lower=log_sig_lower,
                      ...
                      lnM_lower=lnM_lower,
                      Aval_lower=Aval_lower,
                      Bval_lower=Bval_lower,
                      Cval_lower=Cval_lower)
  }
  ...
  if(agecompfit==3)
  {
    lower_bounds <- c(#log_sig_lower=log_sig_lower,
                      ...
                      logit_rhoalphaG_lower=logit_rhoalphaG_lower,
                      lnM_lower=lnM_lower,
                      log_thetaT_lower=log_thetaT_lower,
                      log_thetaG_lower=log_thetaG_lower)
  }
...
setmodel <- function(data,parameters,agecompfit,modelname,mymapinput = list(log_sig=as.factor(NA)))
{
  ...
  # reffects <- c("log_rec")
  # if(agecompfit==1||agecompfit==2){reffects <- c("log_rec")}
  if(agecompfit==1||agecompfit==2||agecompfit==3){reffects <- c("log_rec","log_qG","log_qT")}
  if(agecompfit==4){reffects <- c("log_rec","log_qG","log_qT","log_gamT","log_gamG")}
  
  # MAPPING:
  if(agecompfit==1||agecompfit==3){mymap <- mymapinput
                    # mymap<-list(log_sig=as.factor(NA),
                    #             lnM=as.factor(NA),
                                # log_thetaT=as.factor(NA),
                                # log_thetaG=as.factor(NA))
                    }
  if(agecompfit==2){#mymap <- mymapinput
                    #mymap<-c()
                    mymap<-list(log_sig=as.factor(NA),
                                Aval = as.factor(NA),
                                Bval = as.factor(NA),
                                Cval = as.factor(NA)
                                  # logit_rhoalphaT=as.factor(NA),
                                  # logit_rhoalphaG=as.factor(NA)
                                  # logit_rhoalphacatchT=as.factor(NA),
                                  # logit_rhoalphacatchG=as.factor(NA)
                                  )
                    }
  # mymap = list(log_sig=as.factor(NA))
  ...
  # config(optimize.instantly=0, DLL=modelname)
...
{
  prestheme <- theme_classic()+theme(axis.title=element_text(size=50),axis.text=element_text(size=50),
                                    ...
                                    axis.text.y = element_text(color="black",size=35,angle=90,hjust=0.5))
  ...
  if(checkagainstADMB==T)
  {
    ...
    Abundance.df <- data.frame(Year=rep(ryears,2),Model=rep(c("SCAA","SSSAM"),each=length(ryears)),Abundance=c(Abundance.admb,Abundance))
    Recruitment <- Nmatrix[,1]/1000
    Recruitment.admb <- ADMBdat.table[which(ADMBdat.table[,1]=="Recruitment"),2]/1000
    Recruitment.df <- data.frame(Year=rep(ryears,2),Model=rep(c("SCAA","SSSAM"),each=length(ryears)),Recruitment=c(Recruitment.admb,Recruitment))
    
    jpeg('Abundance.jpg',width=1600,height=900)
    plot(ggplot(data=Abundance.df,aes(x=Year,y=Abundance,color=as.factor(Model)))+
           geom_line(aes(linetype=Model, color=Model),size=4)+
           ...
           prestheme+scale_color_manual(values=c('black',SSMcolor))) 
    dev.off()
    
    jpeg('Recruitment.jpg',width=1600,height=900)
    plot(ggplot(data=Recruitment.df,aes(x=Year,y=Recruitment,color=as.factor(Model)))+
           geom_line(aes(linetype=Model, color=Model),size=4)+
           geom_point(aes(color=Model))+
           xlab("Years")+
           ...
           theme(legend.text = element_text( size = 24))+
           labs(color="Model")+
           ggtitle("Estimated Lake Whitefish Recruitment in WFM03")+ 
           prestheme+scale_color_manual(values=c('black',SSMcolor))) #For presentations
    dev.off()
    #############################################################################
    ...
    Biomass.df <- data.frame(Year=rep(ryears,2),Model=rep(c("SCAA","SSSAM"),each=length(ryears)),Biomass=c(SP_BIO.admb,SP_BIO))
    
    jpeg('Biomass.jpg',width=1600,height=900)
    ...
    SelectivityTrapNetSSM<-ggplot(data=AdjustedqT.df,aes(x=age,y=catchability,color=as.factor(year)))+
      ...
      labs(color="Year")+
      annotate(geom="text",x=5.25, y=1, label="SSSAM (T)",size=24,fontface="bold")+
      prestheme+theme(legend.text = element_text(size = 50),
                      legend.title = element_text(size = 50),
                      axis.text.x = element_text(color="black",size=35),
                      axis.text.y = element_text(color="black",size=35,angle=90,hjust=0.5))
    ...
    SelectivityGillNetSSM<-ggplot(data=AdjustedqG.df,aes(x=age,y=catchability,color=as.factor(year)))+
      ...
      scale_color_manual(values=colorRampPalette(coul)(length(years)))+
      labs(color="Year")+
      annotate(geom="text",x=5.25, y=1, label="SSSAM (G)",size=24,fontface="bold")+
      prestheme+theme(legend.text = element_text(size = 50),
                      legend.title = element_text(size = 50),
                      axis.text.x = element_text(color="black",size=35),
                      axis.text.y = element_text(color="black",size=35,angle=90,hjust=0.5))
    ...
    SelectivityTrapNetSCAAdjusted<-ggplot(data=AdjustedSelT.admb.df,aes(x=age,y=selectivity,color=as.factor(year)))+
      ...
      scale_color_manual(values=colorRampPalette(coul)(length(years)))+
      labs(color="Year")+
      annotate(geom="text",x=ages[2], y=1, label="SCAA (T)",size=24,fontface="bold")+
      prestheme+theme(legend.text = element_text(size = 50),
                     legend.title = element_text(size = 50),
                     axis.text.x = element_text(color="black",size=35),
                     axis.text.y = element_text(color="black",size=35,angle=90,hjust=0.5))
    ...
    SelectivityGillNetSCAAdjusted<-ggplot(data=AdjustedSelG.admb.df,aes(x=age,y=selectivity,color=as.factor(year)))+
      ...
      scale_color_manual(values=colorRampPalette(coul)(length(years)))+
      labs(color="Year")+
      annotate(geom="text",x=ages[2], y=1, label="SCAA (G)",size=24,fontface="bold")+
      prestheme+
    ...
    jpeg('Selectivity.jpg',width=3200,height=1800)
    ...
    CatchabilityGillNetSSM<-ggplot(data=qG.df,aes(x=year,y=catchability,color=as.factor(age)))+
      geom_line(size=1.25)+
      ...
      ylim(0,0.125)+
      labs(color="Age")+
      annotate(geom="text",x=ryears[5], y=0.125, label="SSSAM (G)",size=24,fontface="bold")+
      prestheme+theme(legend.text = element_text(size = 50),
                      legend.title = element_text(size = 50),
                      axis.text.x = element_text(color="black",size=35),
                      axis.text.y = element_text(color="black",size=35,angle=90,hjust=0.5))
    ...
    CatchabilityTrapNetSSM<-ggplot(data=qT.df,aes(x=year,y=catchability,color=as.factor(age)))+
      geom_line(size=1.25)+
      scale_color_manual(values=colorRampPalette(coul)(length(asymptoteages)))+
      xlab("Years")+
      ylab("Catchability (100s of lifts * year)")+
      ylim(0,0.04)+
      labs(color="Age")+
      annotate(geom="text",x=ryears[5], y=0.04, label="SSSAM (T)",size=24,fontface="bold")+
      prestheme+theme(legend.text = element_text(size = 50),
                      legend.title = element_text(size = 50),
                      axis.text.x = element_text(color="black",size=35),
                     axis.text.y = element_text(color="black",size=35,angle=90,hjust=0.5))
    ...
    CatchabilityGillNetSCA<-ggplot(data=qG.admb.df,aes(x=year,y=catchability,color=as.factor(age)))+
      geom_line(size=1.25)+
      scale_color_manual(values=colorRampPalette(coul)(length(ages)))+
      xlab("Years")+
      ylab("Catchability (millions of feet * year)")+
      ylim(0,0.125)+
      labs(color="Age")+
      annotate(geom="text",x=ryears[4], y=0.125, label="SCAA (G)",size=24,fontface="bold")+
    ...
    CatchabilityTrapNetSCA<-ggplot(data=qT.admb.df,aes(x=year,y=catchability,color=as.factor(age)))+
      geom_line(size=1.25)+
      scale_color_manual(values=colorRampPalette(coul)(length(ages)))+
      xlab("Years")+
      ylab("Catchability (100s of lifts * year)")+
      ylim(0,0.04)+
      labs(color="Age")+
      annotate(geom="text",x=ryears[4], y=0.04, label="SCAA (T)",size=24,fontface="bold")+
      # ggtitle("Catchability of Gill Net by Age and Year of Lake Whitefish in WFM03")+
      prestheme+theme(legend.text = element_text(size = 50),
                      legend.title = element_text(size = 50),
                      axis.text.x = element_text(color="black",size=35),
    ...
    Catchabilitylegend <- cowplot::get_legend(CatchabilityTrapNetSCA)
    
    CatchabilityGillNetSCA <- CatchabilityGillNetSCA + theme(legend.position="none")
    CatchabilityTrapNetSCA <- CatchabilityTrapNetSCA + theme(legend.position="none")
    CatchabilityGillNetSSM <- CatchabilityGillNetSSM + theme(legend.position="none")
    CatchabilityTrapNetSSM <- CatchabilityTrapNetSSM + theme(legend.position="none")
    
    
    jpeg('Catchability.jpg',width=3200,height=1800)
    grid.arrange(CatchabilityGillNetSCA,
                 CatchabilityGillNetSSM,
                 Catchabilitylegend,
                 CatchabilityTrapNetSCA,
                 CatchabilityTrapNetSSM,
                 ncol=3,
                 layout_matrix = rbind(c(1,2,3), c(4,5,3)),
                 widths = c(4, 4, 1), heights = c(3,3))
    dev.off() 
    ...
    Mortality<-ggplot(Z.df,aes(ryears,morts))+
      ...
      #ylim(0,1.5)+
      xlab("Years")+
      scale_fill_manual(values=c("deepskyblue4","deepskyblue2","lightskyblue1"))+
      annotate(geom="text",x=ryears[5], y=1.5, label="SSSAM",size=24,fontface="bold")+
      labs(fill="Mortality Type")+
      prestheme+theme(legend.position = "none") #For Presentations
    ...
    MortalityADMB<-ggplot(Z.df.admb,aes(years,morts))+
      ...
      xlab("Years")+
      scale_fill_grey()+
      annotate(geom="text",x=ryears[5], y=1.5, label="SCAA",size=24,fontface="bold")+
      labs(fill="Mortality Type")+
      prestheme+theme(legend.position = "none")
    
    jpeg('Mortality.jpg',width=1600,height=900)
    ...
    PredT.df<- data.frame(Year=rep(ryears),Model=rep(c("SCAA","SSSAM"),each=length(ryears)),Prediction=c(PredT.admb,PredT))
    PredG.df<- data.frame(Year=rep(ryears),Model=rep(c("SCAA","SSSAM"),each=length(ryears)),Prediction=c(PredG.admb,PredG))
    
    PredictedTrapHarvest<-ggplot(data=PredT.df,aes(x=Year,y=Prediction,color=as.factor(Model)))+
      geom_line(data=data.frame(years,ObsT),aes(x=years,y=ObsT),color="black",size=4)+
      geom_line(aes(color=Model),size=4,linetype="dotted")+
      scale_color_manual(values=c('red','blue'))+
      xlab("Years")+
      ylab("Harvest (x1,000)")+ylim(0,600)+
      theme(legend.text = element_text( size = 24))+
      annotate(geom="text",x=ryears[1], y=600, label="Trap",size=24,fontface="bold")+
      labs(color="Model")+
      prestheme+theme(axis.text.x = element_text(color="black",size=35),
                      axis.text.y = element_text(color="black",size=35,angle=90,hjust=0.5),
                      legend.text = element_text(size = 50),
                      legend.title = element_text(size = 50))
    
    PredictedGillHarvest<-ggplot(data=PredG.df,aes(x=Year,y=Prediction,color=as.factor(Model)))+
      geom_line(data=data.frame(years,ObsG),aes(x=years,y=ObsG),color="black",size=4)+
      geom_line(aes(color=Model),size=4,linetype="dotted")+
      scale_color_manual(values=c('red','blue'))+
      xlab("Years")+
      ylab("Harvest (x1,000)")+ylim(0,600)+
      theme(legend.text = element_text( size = 24))+
      annotate(geom="text",x=ryears[1], y=600, label="Gill",size=24,fontface="bold")+
      labs(color="Model")+
      prestheme+theme(axis.text.x = element_text(color="black",size=35),
                      axis.text.y = element_text(color="black",size=35,angle=90,hjust=0.5),
                      legend.text = element_text(size = 50),
    ...
    jpeg('Predicted.jpg',width=3200,height=1800)
    grid.arrange(PredictedGillHarvest,PredictedTrapHarvest,Predictedlegend,ncol=2,
...
savegraphspublication<-function(checkagainstADMB=F,oridinalwd,newwd,sdr,data)
{
  ...
  plot(ggplot(data=Effort.df,aes(x=Year,y=Effort,color=as.factor(Fishery)))+
         ...
         geom_point(size=3)+
         xlab("Years")+
         ylab("Effort (Millions of Feet or 100s of Lifts)")+
  ...
  if(checkagainstADMB==T)
  {
    ...
    Traplabel <- ggplot()+annotate(geom="text",x=10, y=10, label="Trap Net Fishery",size=24,fontface="bold")+theme_void()
    Gilllabel <- ggplot()+annotate(geom="text",x=10, y=10, label="Gill Net Fishery",size=24,fontface="bold")+theme_void()
    SSMlabel <- ggplot()+annotate(geom="text",x=10, y=10, label="SSM",size=24,fontface="bold",angle=90)+theme_void()
    SCAlabel <- ggplot()+annotate(geom="text",x=10, y=10, label="SCA",size=24,fontface="bold",angle=90)+theme_void()
    ...
    CatchabilityGillNetSSM<-ggplot(data=qG.df,aes(x=year,y=catchability,color=as.factor(age)))+
           ...
           ylim(0,0.125)+
           labs(color="Age")+
           annotate(geom="text",x=ryears[1], y=0.125, label="B",size=24,fontface="bold")+
           pubtheme+theme(legend.text = element_text( size = 24))
    ...
    CatchabilityTrapNetSSM<-ggplot(data=qT.df,aes(x=year,y=catchability,color=as.factor(age)))+
           ...
           ylim(0,0.04)+
           labs(color="Age")+
           annotate(geom="text",x=ryears[1], y=0.04, label="D",size=24,fontface="bold")+
           pubtheme+theme(legend.text = element_text( size = 24))
    ...
    CatchabilityGillNetSCA<-ggplot(data=qG.admb.df,aes(x=year,y=catchability,color=as.factor(age)))+
           ...
           ylim(0,0.125)+
           labs(color="Age")+
           annotate(geom="text",x=ryears[1], y=0.125, label="A",size=24,fontface="bold")+
           # ggtitle("Catchability of Gill Net by Age and Year of Lake Whitefish in WFM03")+
           pubtheme+theme(legend.text = element_text( size = 24))
    ...
    CatchabilityTrapNetSCA<-ggplot(data=qT.admb.df,aes(x=year,y=catchability,color=as.factor(age)))+
           ...
           ylim(0,0.04)+
           labs(color="Age")+
           annotate(geom="text",x=ryears[1], y=0.04, label="C",size=24,fontface="bold")+
           # ggtitle("Catchability of Gill Net by Age and Year of Lake Whitefish in WFM03")+
           pubtheme+
           theme(legend.text = element_text(size = 36),
                 legend.title = element_text(size = 36))
    ...
    jpeg('Catchability.jpg',width=2400,height=1800)
    grid.arrange(Blank,
                 Gilllabel,
                 Traplabel,
                 Catchabilitylegend,
                 SCAlabel,
                 CatchabilityGillNetSCA,
                 CatchabilityGillNetSSM,
                 SSMlabel,
                 CatchabilityTrapNetSCA,
                 CatchabilityTrapNetSSM,
